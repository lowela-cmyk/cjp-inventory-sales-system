...
class AdminRevenueInsightController extends Controller
{
    public function __invoke(Request $request, RevenueInsightService $insights): RedirectResponse
    {
        ...
        if ($result['ok']) {
            return $redirect->with('revenueInsight', [
                'text' => $result['text'],
                'generated_at' => now()->format('M d, Y h:i A'),
            ]);
        }

        return $redirect->with('revenueInsightNotice', $result['text']);
    }
    ...
    private function aiFilters(array $filters): array
    {
        ...
        return [
            ...
            'trend_year' => (int) ($filters['period'] === 'month'
                ? CarbonImmutable::parse($filters['month'].'-01')->year
                : $filters['year']),
            'expected_year' => (int) ($filters['period'] === 'month'
                ? CarbonImmutable::parse($filters['month'].'-01')->year
                : $filters['year']),
            'limit' => 3,
        ];

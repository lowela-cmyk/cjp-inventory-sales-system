...
class AdminSalesTrendSummaryController extends Controller
{
    public function __invoke(Request $request, SalesTrendSummaryService $summaries): RedirectResponse
    {
        $filters = $this->validatedFilters($request);
        $result = $summaries->generateForUser($request->user(), $this->aiFilters($filters));

        $redirect = redirect()->route('admin.reports', $filters);

        if ($result['ok']) {
            return $redirect->with('salesTrendSummary', [
                'text' => $result['text'],
                'generated_at' => now()->format('M d, Y h:i A'),
            ]);
        }

        return $redirect->with('salesTrendSummaryNotice', $result['text']);
    }
